...
def send_email(path,new_path,file_name,fromadd,toadd,ccadd,Sub,Body,Smtp,pwd):
    pythoncom.CoInitialize()
    lock.acquire()
    excelFile = file_name
...
if __name__ == '__main__':
    ...
    for i in range(x.shape[0]):
        ...
        t = threading.Thread(target=send_email,args=(path,back_path,data[i][9],data[i][3],data[i][4],data[i][5],data[i][1],data[i][2],data[i][6],data[i][8]))
        threadu.append(u)
        threadt.append(t)
    for thru in threadu:
        thru.start()
